...
class ChatProvider extends ChangeNotifier {
  ...
  final BridgefyTransmissionModeType transmissionModeType =
      BridgefyTransmissionModeType.broadcast;
  ...
  void setInitialMessages(String? encodedMessages) {
    if (encodedMessages == null || encodedMessages.isEmpty) {
      return;
    }

    final List decodedMessages =
        json.decode(Uri.decodeComponent(encodedMessages));
    messageList = decodedMessages
        .map((msg) => Message(
              text: msg['text'],
              origin: msg['user']['_id'] == userId
                  ? OriginMessage.me
                  : OriginMessage.other,
            ))
        .toList();
